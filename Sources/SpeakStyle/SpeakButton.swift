...
public struct SpeakButton: View {
    ...
    public init(text : String) {
        self.text = text
        self.colour = Color.accentColor
    }
    public init(text : String,colour : Color) {
        self.text = text
        self.colour = colour
    }

    public var body: some View {
        Label(text, systemImage: "speaker.1.fill").labelStyle(.titleAndIcon).frame(maxHeight: 1).padding().foregroundColor(.white).background(colour).cornerRadius(20).speakable(withText: text)
